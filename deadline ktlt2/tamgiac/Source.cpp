...
int tinhSTTNgayTrongNam(SNgay a)
{
	...
	for (int thang = 1; thang < a.thang; thang++)//sai------------ vi khi truyen vao ham thi thang k co nam nen k kt dk năm nhuan
	{
...
void timNgayHomTruoc(SNgay &a, int k)
{
	...
	if (a.ngay <= 0)
	{
		...
		a.ngay = tinhSoNgayTrongThang(a) + a.ngay; // cong voi so ngay am khi tru
	}
}
...
void timNgayHomSau(SNgay &a, int k)
{
	...
	{
		a.ngay = a.ngay - tinhSoNgayTrongThang(a);
		a.thang++;
		if (a.thang > 12)
		{
			a.thang = 1;
