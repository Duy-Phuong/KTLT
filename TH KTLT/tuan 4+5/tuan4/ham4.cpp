...
//bai 2: viet chuong trinh nhap vao so nguyen va xuat so do ra o dang chuoi, nhung co dau phay ngan o hang ngan, hang tram, hang trieu for
...
void ChuyenSoSangChuoi()
{
	...
	for (int i = x; i > 0; i--)
	{
		...
		if (dem != 0 && dem % 3 == 0)
		{
			ChenKiTu(a, i - 1, ',');// chen o vi tri i-1 vi khi chen vao vi tri i nó chen o sau i
		}
	}
	if (a[0] == ',')
	{
		Xoa(a, 0);//neu o dau chuoi co dau phay ta xoa
	}
	puts(a);
	free(a);
}
...
void ChuanHoaDoanVan(char* s)
{
	...
	XoaKhoangTrangThua(a);//sau khi chen no co the du ra
	VietHoaKiTuDauSauDauCau(a);
	puts(a);
}

//cau b:
void demsocau(char* a)
{
	int n = strlen(a);
	int dem = 1;
	for (int i = 0; i < n; i++)
...
bool KiemTraHopLe(char* s)
{
	...
	while (i < n)
	{
		...
		if (s[i] == ')')
		{
			...
			dem = 0;
			i = j;
			i--;//vi sau khi cho i=j i++ lan nua se sai
		}
